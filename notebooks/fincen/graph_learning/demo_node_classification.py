...
train_fincen_graph = load_graphs("/dbfs/mnt/public/clean/fincen-graph/train_fincen_graph")[0][0]
...
train_mask = th.zeros(len(fincen_graph.nodes()), dtype=th.bool).reshape(len(fincen_graph.nodes() ), 1) 
train_mask[train_nodes] = 1 

test_mask = th.zeros(len(fincen_graph.nodes()), dtype=th.bool).reshape(len(fincen_graph.nodes() ),1)
...
for e in range(epochs):
  ...
  loss = F.binary_cross_entropy_with_logits(ex_score[train_mask_flattened], labels[train_mask_flattened])

  # Compute accuracy on training/test
  train_acc = (pred[train_mask_flattened] == labels[train_mask_flattened]).float().mean()
